...
namespace WorldCitiesAPI.Controllers
{
    ...
    public class SeedController : ControllerBase
    {
        ...
        {
            // prevents non-development environments from running
            ...
            for (int nRow = 2; nRow <= nEndRow; nRow++)
            {
                ...
                var countryName = row[nRow, 5].GetValue<string>();
                // retrieve the country ID by countryName
                var countryId = countriesByName[countryName].Id;
                // skip this city if it already exists in the database
                if (cities.ContainsKey((Name: name, Lat: lat, Lon: lon, CountryId: countryId))) { continue; }
                var city = new City
                {
                    Name = name,
                    Lat = lat,
            ...
            else { return new NotFoundObjectResult("No new cities found!"); }
        }
    }
}
